...
class CameraService:
    ...
    * `_jpeg_lock` guards the single shared JPEG encode.
    ...
    def __init__(self, camera_index: int = DEFAULT_CAMERA_INDEX):
        ...
        self._cached_cameras: Optional[List[Dict[str, Any]]] = None

        self.start()

    def list_cameras(self, max_test: int = 4, force_refresh: bool = False) -> List[Dict[str, Any]]:
        if self._cached_cameras is not None and not force_refresh:
            for c in self._cached_cameras:
                c["active"] = (c["index"] == self.camera_index)
        ...
        for i in range(max_test):
            if i == self.camera_index and self.cap is not None and self.cap.isOpened():
                ...
                try:
                    test_cap = cv2.VideoCapture(i, cv2.CAP_DSHOW if platform.system() == "Windows" else cv2.CAP_ANY)
                    if test_cap.isOpened():
                        cams.append({"index": i, "name": f"Camera {i}", "active": (i == self.camera_index)})
                        test_cap.release()
                except Exception:
                    pass
        if not cams:
            cams.append({"index": self.camera_index, "name": f"Camera {self.camera_index}", "active": True})
